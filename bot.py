...
def api_admin_orders():
    ...
    for o in chunk:
        result.append({
            'code':        o.get('code', ''),
            'product':     products.get(o.get('product_id',''), {}).get('name', '—'),
...
def api_admin_extend(pid):
    ...
    if not p: return jsonify({'error':'Not found'}), 404
    try:
        dt = datetime.strptime(p['deadline_dt'], '%Y-%m-%d %H:%M') + timedelta(hours=24)
    except:
        dt = datetime.now() + timedelta(hours=24)
    p['deadline']    = dt.strftime('%d.%m.%Y %H:%M')
    p['deadline_dt'] = dt.strftime('%Y-%m-%d %H:%M')
    save_data()
    return jsonify({'ok': True, 'deadline': p['deadline']})
...
def api_admin_sellers():
    ...
    for uid, pids in seller_products.items():
        ...
        # Use seller_shops name if available
        shops_data = seller_shops.get(uid, seller_shops.get(int(uid) if str(uid).isdigit() else uid, []))
        shop_names = [s.get('name', '') for s in shops_data] if shops_data else []
        result.append({
            'uid':        str(uid),
...
def setup_menu_route():
    ...
    if BUYER_TOKEN and miniapp_url:
        r = requests.post(f'https://api.telegram.org/bot{BUYER_TOKEN}/setChatMenuButton', json={
            'menu_button': {'type': 'web_app', 'text': '🛍 Joynshop', 'web_app': {'url': miniapp_url}}
        }).json()
    ...
    if SELLER_TOKEN:
        r = requests.post(f'https://api.telegram.org/bot{SELLER_TOKEN}/setChatMenuButton', json={
            'menu_button': {'type': 'commands'}
        }).json()
